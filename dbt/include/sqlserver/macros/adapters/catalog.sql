{#
    Shared body of the catalog queries: every table and view in the current
    database, one row per column, with owners and MS_Description comments.
    Callers append their own WHERE clause to scope the result, followed by
    the ORDER BY and query label.
#}
{% macro sqlserver__catalog_tables_and_columns_sql() -%}
        with
        principals as (
            select
...
{% macro sqlserver__get_catalog(information_schemas, schemas) -%}
    ...
    {%- call statement('catalog', fetch_result=True) -%}
        ...
        {{ sqlserver__catalog_tables_and_columns_sql() }}
